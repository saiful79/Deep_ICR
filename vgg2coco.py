...
path = f"/media/sayan/hdd1/DATASET/SROIE_datasetv2/SROIE2019/{data_type}/vgg_annotation.json"
image_dir = f"/media/sayan/hdd1/DATASET/SROIE_datasetv2/SROIE2019/{data_type}/img" 

################### output file ##########################333

# OUTPUT_TXT_FILE = f"/media/sayan/hdd1/PROJECT/MRZ_DETECTION/training/dataset/{data_type}.txt"
OUTPUT_JSON_FILE = f"/media/sayan/hdd1/DATASET/SROIE_datasetv2/SROIE2019/{data_type}/{data_type}.json"
            ...
            if os.path.exists(img_path):
                ...
                for region in data[i]['regions']:
                    ...
                    if region['shape_attributes']['name'] =='rect':
                        ...
                        x = min(all_x)
                        y = min(all_y)
                        w = abs(max(all_x)-min(all_x))
                        h = abs(max(all_y)-min(all_y))  
                    else:
                        pass
